...
class MeanShift(nn.Conv2d):
    def __init__(
        self, rgb_range,
        rgb_mean=(0.33, 0.0, 0.0), rgb_std=(1.0, 1.0, 1.0), sign=-1):
...
class RLKAN(nn.Module):
    def __init__(self, n_resblocks=4, RLS_stage = 4, n_resgroups=1, n_colors=3, n_feats=180, scale=2, res_scale = 1.0, bit = 8):
        super(RLKAN, self).__init__()

        #res_scale = res_scale
        self.n_resgroups = n_resgroups
        self.Stage = RLS_stage
